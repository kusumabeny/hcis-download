...
Kartu iOS PWA juga otomatis membaca entry terbaru dan mengisi riwayat versi dari `CHANGELOG-PWA.md` repository HCIS-mobile. Data release lokal dikirim segera agar halaman tidak menunggu GitHub; pembaruan changelog berjalan di background dan di-cache server selama 5 menit. Jika GitHub sementara tidak dapat diakses, halaman tetap memakai data release tersimpan. Lokasi dapat diubah melalui `GITHUB_PWA_CHANGELOG_REPO`, `GITHUB_PWA_CHANGELOG_PATH`, dan `GITHUB_PWA_CHANGELOG_BRANCH`.
